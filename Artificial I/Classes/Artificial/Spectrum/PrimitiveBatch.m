...
- (void) beginWithBlendState:(BlendState*)theBlendState 
	...
	if (theTransformMatrix && [effect isKindOfClass:[BasicEffect class]]) {
...
- (void) drawPointAt:(Vector2 *)position color:(Color *)color layerDepth:(float)layerDepth {
	[self drawLineFrom:[Vector2 vectorWithX:position.x-0.0001f y:position.y-0.0001f]
					to:[Vector2 vectorWithX:position.x+0.0001f y:position.y+0.0001f] color:color layerDepth:layerDepth];	
}
...
- (void) drawRectangle:(Rectangle*)rectangle color:(Color*)color {
	[self drawRectangle:rectangle color:color layerDepth:0];
}

- (void) drawRectangle:(Rectangle*)rectangle color:(Color*)color layerDepth:(float)layerDepth {
	[self drawRectangleAt:[Vector2 vectorWithX:rectangle.x +rectangle.width/2 
											 y:rectangle.y + rectangle.height/2] 
					width:rectangle.width height:rectangle.height color:color layerDepth:layerDepth];
}

- (void) draw3DLineFrom:(Vector3*)start to:(Vector3*)end color:(Color*)color {
	vertex.color = color.packedValue;
	
	vertex.position = *start.data;
	[vertexArray addVertex:&vertex];
	
	vertex.position = *end.data;
	[vertexArray addVertex:&vertex];
